...
// Feature-store
const fcs = require("./src/feature-store.js")
...
var geojson = fileio.readLocalSync("mangrove-atlas-data.geojson")
// FeatureCollection ID
var fcid = "test-feature-collection"
// FeatureCollection MetaData
var cmd = fcs.createCollectionMetaData({
  "name": "An example feature collection"
  })

// Create FeatureStore and add GeoJSON FeatureCollection
var fstore = fcs.createFeatureStore("./fstore")
.createFeatureCollection(fcid, cmd)
.createFeaturesFromGeoJSON(fcid, geojson, index_names=["iso", "type", "id"])

// Create FeatureStore and add GeoJSON FeatureCollection
var fstore = fcs.createFeatureStore("./fstore")
.createFeatureCollection(fcid + "2", cmd)
.createFeaturesFromGeoJSON(fcid + "2", geojson, index_names=["iso", "type", "id"])

// Make query to store
var res = fstore
.queryFeatureCollection(fcid, {"iso":"TZA", "type":"country"}, include_features=true, include_geometries=true)
console.log("\nQuery results:", res)



/**
 * Feature-store RESTful API demonstration
 * 
 * @author Edward P. Morris (vizzuality)
 * 
 * @description This code contains the main feature-store RESTful API service
 * demonstration
 * 
 */
...
// To change edit .env file, available in JS as process.env.MY_VARIABLE
...
exports.fstore = (req, res) => {
  ...
  // Define parameters used in service
  ...
  // @param {!string} fid FeatureCollection ID
  var fcid = uts.parseRequest(req.query, 'fcid', 'String', true, null)
